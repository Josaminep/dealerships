...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_receipt'])) {
    ...
    $delete_query = $db->prepare("DELETE FROM receipts3 WHERE id3 = ?");
...
// Fetch all receipts
$receipts = $db->query("SELECT r.id3, r.receipt_details3, r.created_at3, s.total_price3, p.product_name3 
                        FROM receipts3 r
                        JOIN sales3 s ON r.sale_id3 = s.id3
                        JOIN products3 p ON s.product_id3 = p.id3
                        ORDER BY r.created_at3 DESC")->fetchAll();
?>
...
<form id="selectionForm">
    ...
    <tr>
        <td><?php echo htmlspecialchars($receipt['id3']); ?></td>
        <td><?php echo htmlspecialchars($receipt['product_name3']); ?></td>
        <td><?php echo htmlspecialchars($receipt['receipt_details3']); ?></td>
        <td><?php echo htmlspecialchars($receipt['created_at3']); ?></td>
        <td>
            <button class="print-button" onclick="printReceipt(<?php echo $receipt['id3']; ?>)">Print</button>
            <form method="POST" style="display:inline;">
                <input type="hidden" name="receipt_id" value="<?php echo $receipt['id3']; ?>">
