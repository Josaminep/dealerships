...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_product'])) {
        ...
        if ($stmt->execute([$product_name3, $price3, $quantity3, $brand3, $stock3])) {
    ...
    if (isset($_POST['add_stock'])) {
        ...
        $additional_stock3 = $_POST['additional_stock3'];

        $stmt = $db->prepare("UPDATE products3 SET stock3 = stock3 + ? WHERE id3 = ?");
        if ($stmt->execute([$additional_stock3, $product_id3])) {
    ...
    if (isset($_POST['modify_price'])) {
        ...
        $product_id3 = $_POST['product_id3'];
        $new_price3 = $_POST['new_price3'];

        $stmt = $db->prepare("UPDATE products3 SET price3 = ? WHERE id3 = ?");
        if ($stmt->execute([$new_price3, $product_id3])) {
...
$products3 = $db->query("SELECT * FROM products3")->fetchAll(PDO::FETCH_ASSOC);

// Handle delete request
if (isset($_POST['delete_product'])) {
    $product_id3 = $_POST['product_id3'];
    $db->prepare("DELETE FROM products3 WHERE id3 = ?")->execute([$product_id3]);
...
if ($search) {
    if (strtolower($search) == 'lowstocks') {
        ...
        $stmt = $db->prepare("SELECT * FROM products3 WHERE stock3 < 10");
        $stmt->execute();
    } else {
        // Fetch products based on product name or description (brand)
        $stmt = $db->prepare("SELECT * FROM products3 WHERE product_name3 LIKE ? OR brand3 LIKE ?");
        $stmt->execute(['%' . $search . '%', '%' . $search . '%']);
    }
    $products3 = $stmt->fetchAll(PDO::FETCH_ASSOC);
} else {
...
<div class="content">
    ...
    <div class="main-content">
        <div class="manage_products">
            <h3>Branch 3</h3>
            <div class="branch3_add_stocks">
                <!-- Add New Product Form -->
                <div class="form-container">
                    <h3>Add New Product in Branch 3</h3>
                    <form method="POST" action="">
                        <input type="text" name="product_name3" placeholder="Product Name" required>
                        <input type="number" step="0.01" name="price3" placeholder="Price" required>
                        <input type="number" name="quantity3" placeholder="Quantity" required>
                        <input type="text" name="brand3" placeholder="Brand" required>
                        <input type="number" name="stock3" placeholder="Stock" required>
                ...
                <form method="POST" class="form-container">
                    ...
                    <select name="product_id3" required>
                    <h3>Add Stocks</h3>
                        <option value="">Select Product</option>
                        <?php foreach ($products3 as $product): ?>
                            <option value="<?php echo $product['id3']; ?>"><?php echo $product['product_name3']; ?></option>
                        <?php endforeach; ?>
                    </select>
                    <input type="number" name="additional_stock3" placeholder="Additional Stock" required>
                ...
                <h3>Modify Price</h3>
                    ...
                    <input type="number" name="new_price3" placeholder="New Price" required>
    ...
    <table>
        ...
        <?php foreach ($products3 as $product): ?>
            <tr>
                ...
                <td><?php echo htmlspecialchars($product['stock3']); ?></td>
                <td>
                    <form method="POST" style="display:inline;">
                        <input type="hidden" name="product_id3" value="<?php echo $product['id3']; ?>">
