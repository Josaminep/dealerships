...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($_POST['product_id'] as $index => $product_id) {
        ...
        $stmt = $db->prepare("SELECT * FROM products2 WHERE id2 = ?");
        $stmt->execute([$product_id]);
        $product = $stmt->fetch();

        if ($product && $product['stock2'] >= $quantity_sold) {
            // Calculate total price for this product
            $price = $quantity_sold * $product['price2'];
            $total_price += $price;
            ...
            $products_info[] = "{$quantity_sold} {$product['product_name2']} at Php {$product['price2']} each"; // Added price
        } else {
            echo "Insufficient stock for the product: {$product['product_name2']}!";
            exit();
        }
    }
   ...
   foreach ($sales_data as $data) {
    $stmt = $db->prepare("INSERT INTO sales2 (product_id2, quantity_sold2, total_price2, sale_date2) VALUES (?, ?, ?, NOW())");
   ...
   $stmt = $db->prepare("INSERT INTO receipts2 (sale_id2, receipt_details2) VALUES (?, ?)");
   $stmt->execute([$sale_ids[0], $receipt_details]); // Use the first sale ID for the receipt
    ...
    $stmt = $db->prepare("INSERT INTO income_records2 (period2, amount2) VALUES ('daily', ?) ON DUPLICATE KEY UPDATE amount2 = amount2 + ?");
    $stmt->execute([$total_price, $total_price]);

    $stmt = $db->prepare("INSERT INTO income_records2 (period2, amount2) VALUES ('weekly', ?) ON DUPLICATE KEY UPDATE amount2 = amount2 + ?");
    $stmt->execute([$total_price, $total_price]);

    $stmt = $db->prepare("INSERT INTO income_records2 (period2, amount2) VALUES ('monthly', ?) ON DUPLICATE KEY UPDATE amount2 = amount2 + ?");
    $stmt->execute([$total_price, $total_price]);
        ...
        <form method="POST">
            <div id="products-container">
                <div>
                    ...
                    <select name="product_id[]" required>
    <?php foreach ($products as $product): ?>
        <option value="<?php echo $product['id2']; ?>">
            <?php echo $product['product_name2'] . " (Php " . number_format($product['price2'], 2) . ", Stock: " . $product['stock2'] . ")"; ?>
        </option>
    <?php endforeach; ?>
</select>
