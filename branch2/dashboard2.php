...
$daily_income_query = $db->prepare("SELECT SUM(total_price2) FROM sales2 WHERE sale_date2 >= ?");
$weekly_income_query = $db->prepare("SELECT SUM(total_price2) FROM sales2 WHERE sale_date2 >= ?");
$monthly_income_query = $db->prepare("SELECT SUM(total_price2) FROM sales2 WHERE sale_date2 >= ?");
$yearly_income_query = $db->prepare("SELECT SUM(total_price2) FROM sales2 WHERE sale_date2 >= ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['reset_all'])) {
        ...
        $db->exec("UPDATE sales2 SET total_price2 = 0 WHERE sale_date2 >= '$today'");
    } elseif (isset($_POST['reset_daily'])) {
        $db->exec("UPDATE sales2 SET total_price2 = 0 WHERE sale_date2 >= '$today'");
    } elseif (isset($_POST['reset_weekly'])) {
        $db->exec("UPDATE sales2 SET total_price2 = 0 WHERE sale_date2 >= '$this_week'");
    } elseif (isset($_POST['reset_monthly'])) {
        $db->exec("UPDATE sales2 SET total_price2 = 0 WHERE sale_date2 >= '$this_month'");
    }
...
for ($i = 0; $i < 12; $i++) {
    ...
    $monthly_sales_query = $db->prepare("SELECT SUM(total_price2) FROM sales2 WHERE sale_date2 >= ? AND sale_date2 < DATE_ADD(?, INTERVAL 1 MONTH)");
...
for ($i = 0; $i < 5; $i++) {
    ...
    $yearly_sales_query = $db->prepare("SELECT SUM(total_price2) FROM sales2 WHERE sale_date2 >= ? AND sale_date2 < DATE_ADD(?, INTERVAL 1 YEAR)");
...
$product_stocks = $db->query("SELECT product_name2, stock2 FROM products2")->fetchAll(PDO::FETCH_ASSOC);

// Fetch activity log records
$activity_log_query = $db->query("SELECT * FROM activity_log2 ORDER BY activity_time2 DESC LIMIT 20");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_product'])) {
    ...
    $query = $db->prepare("INSERT INTO products2 (product_name2, price2, quantity2, brand2, stock2) VALUES (?, ?, ?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['make_purchase'])) {
    ...
    // Fetch product info
    $product_query = $db->prepare("SELECT product_name2 FROM products2 WHERE id2 = ?");
    ...
    $price_query = $db->prepare("SELECT price2 FROM products2 WHERE id2 = ?");
    $price_query->execute([$product_id]);
    $product_price = $price_query->fetchColumn();

    // Add purchase to sales table
    $total_price = $product_price * $quantity;
    $insert_sale = $db->prepare("INSERT INTO sales2 (product_id2, quantity_sold2, sale_date2, total_price2) VALUES (?, ?, ?, ?)");
    $insert_sale->execute([$product_id, $quantity, date('Y-m-d'), $total_price]);

    // Log the activity
    log_activity($db, 'Purchase', "Purchased $quantity of '{$product['product_name2']}'.");
...
<div class="count">
    ...
    <script>
        ...
        // Product Stock Chart
        const productLabels = <?php echo json_encode(array_column($product_stocks, 'product_name2')); ?>;
        const productStocks = <?php echo json_encode(array_column($product_stocks, 'stock2')); ?>;
