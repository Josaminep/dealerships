...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_product'])) {
        ...
        $stmt = $db->prepare("INSERT INTO products4 (product_name4, price4, quantity4, brand4, stock4) VALUES (?, ?, ?, ?, ?)");
        if ($stmt->execute([$product_name4, $price4, $quantity4, $brand4, $stock4])) {
    ...
    if (isset($_POST['add_stock'])) {
        ...
        $additional_stock4 = $_POST['additional_stock4'];

        $stmt = $db->prepare("UPDATE products4 SET stock4 = stock4 + ? WHERE id4 = ?");
        if ($stmt->execute([$additional_stock4, $product_id4])) {
    ...
    if (isset($_POST['modify_price'])) {
        ...
        $product_id4 = $_POST['product_id4'];
        $new_price4 = $_POST['new_price4'];

        $stmt = $db->prepare("UPDATE products4 SET price4 = ? WHERE id4 = ?");
        if ($stmt->execute([$new_price4, $product_id4])) {
...
$products4 = $db->query("SELECT * FROM products4")->fetchAll(PDO::FETCH_ASSOC);

// Handle delete request
if (isset($_POST['delete_product'])) {
    $product_id4 = $_POST['product_id4'];
    $db->prepare("DELETE FROM products4 WHERE id4 = ?")->execute([$product_id4]);
...
if ($search) {
    if (strtolower($search) == 'lowstocks') {
        ...
        $stmt = $db->prepare("SELECT * FROM products4 WHERE stock4 < 10");
        $stmt->execute();
    } else {
        // Fetch products based on product name or description (brand)
        $stmt = $db->prepare("SELECT * FROM products4 WHERE product_name4 LIKE ? OR brand4 LIKE ?");
        $stmt->execute(['%' . $search . '%', '%' . $search . '%']);
    }
    $products4 = $stmt->fetchAll(PDO::FETCH_ASSOC);
} else {
...
<div class="sidebar">
    ...
    <ul>
        ...
        <li><a href='./add_product4.php'>PRODUCTS</a></li>
...
<div class="content">
    ...
    <div class="main-content">
        <div class="manage_products">
            <h3>Branch 4</h3>
            <div class="branch4_add_stocks">
                <!-- Add New Product Form -->
                <div class="form-container">
                    <h3>Add New Product in Branch 4</h3>
                    <form method="POST" action="">
                        <input type="text" name="product_name4" placeholder="Product Name" required>
                        <input type="number" step="0.01" name="price4" placeholder="Price" required>
                        <input type="number" name="quantity4" placeholder="Quantity" required>
                        <input type="text" name="brand4" placeholder="Brand" required>
                        <input type="number" name="stock4" placeholder="Stock" required>
                        <button type="submit" name="add_product">Add Product</button>
                    </form>
                </div>

                <form method="POST" class="form-container">
                    <select name="product_id4" required>
                        <h3>Add Stocks</h3>
                        <option value="">Select Product</option>
                        <?php foreach ($products4 as $product): ?>
                            <option value="<?php echo $product['id4']; ?>"><?php echo $product['product_name4']; ?></option>
                        <?php endforeach; ?>
                    </select>
                    <input type="number" name="additional_stock4" placeholder="Additional Stock" required>
                ...
                <form method="POST" class="form-container">
                    ...
                    <input type="number" name="new_price4" placeholder="New Price" required>
...
<div class="view_stocks">
    ...
    <table>
        ...
        <?php foreach ($products4 as $product): ?>
            <tr>
                <td><?php echo htmlspecialchars($product['product_name4']); ?></td>
                <td>Php <?php echo number_format($product['price4'], 2); ?></td>
                <td><?php echo htmlspecialchars($product['quantity4']); ?></td>
                <td><?php echo htmlspecialchars($product['brand4']); ?></td>
                <td><?php echo htmlspecialchars($product['stock4']); ?></td>
                <td>
                    <form method="POST" style="display:inline;">
                        <input type="hidden" name="product_id4" value="<?php echo $product['id4']; ?>">
