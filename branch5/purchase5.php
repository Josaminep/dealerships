...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($_POST['product_id'] as $index => $product_id) {
        ...
        $stmt = $db->prepare("SELECT * FROM products5 WHERE id5 = ?");
        $stmt->execute([$product_id]);
        $product = $stmt->fetch();

        if ($product && $product['stock5'] >= $quantity_sold) {
            // Calculate total price for this product
            $price = $quantity_sold * $product['price5'];
            $total_price += $price;
            ...
            $products_info[] = "{$quantity_sold} {$product['product_name5']} at Php {$product['price5']} each"; // Added price
        } else {
            echo "Insufficient stock for the product: {$product['product_name5']}!";
            exit();
        }
    }
    ...
    foreach ($sales_data as $data) {
        $stmt = $db->prepare("INSERT INTO sales5 (product_id5, quantity_sold5, total_price5, sale_date5) VALUES (?, ?, ?, NOW())");
    ...
    $stmt = $db->prepare("INSERT INTO receipts5 (sale_id5, receipt_details5) VALUES (?, ?)");
    ...
    // Record income in income_records5
    $stmt = $db->prepare("INSERT INTO income_records5 (period5, amount5, date) VALUES ('daily', ?, CURDATE()) ON DUPLICATE KEY UPDATE amount5 = amount5 + ?");
    $stmt->execute([$total_price, $total_price]);

    $stmt = $db->prepare("INSERT INTO income_records5 (period5, amount5, date) VALUES ('weekly', ?, CURDATE()) ON DUPLICATE KEY UPDATE amount5 = amount5 + ?");
    $stmt->execute([$total_price, $total_price]);

    $stmt = $db->prepare("INSERT INTO income_records5 (period5, amount5, date) VALUES ('monthly', ?, CURDATE()) ON DUPLICATE KEY UPDATE amount5 = amount5 + ?");
    $stmt->execute([$total_price, $total_price]);

    // Log the activity
    $activity_details = "Admin processed a purchase of total Php {$total_price}.";
    $stmt = $db->prepare("INSERT INTO activity_log5 (activity_type5, details5) VALUES ('Purchase', ?)");
        ...
        <form method="POST">
            <div id="products-container">
                <div>
                    ...
                    <select name="product_id[]" required>
    <?php foreach ($products as $product): ?>
        <option value="<?php echo $product['id5']; ?>">
            <?php echo $product['product_name5'] . " (Php " . number_format($product['price5'], 2) . ", Stock: " . $product['stock5'] . ")"; ?>
        </option>
    <?php endforeach; ?>
</select>
